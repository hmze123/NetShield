		...
		try
		{
			...
			for (String dns : dnsResolver) {
				try {
					...
					mHostService.onDiagnosticMessage(String.format("Failed to add dns %s, %s", dns, iae.getLocalizedMessage()));
				}
			}
			...
			if (Build.VERSION.SDK_INT >= 21) {
				if (useBypass) {
					mHostService.onDiagnosticMessage("<strong><font color='#2AFF0D'>SlowDNS Bypass OK</font></strong>");
