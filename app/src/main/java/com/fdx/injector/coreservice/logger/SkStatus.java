package com.fdx.injector.coreservice.logger;
...
public class SkStatus
{
    ...
    public synchronized static void clearPingLogs() {
        Iterator<LogItem> iterator = logbuffer.iterator();

        while (iterator.hasNext()) {
            LogItem log = iterator.next();

            if (log.getMessage() != null && (log.getMessage().contains("Conexi├│n Enviado") || (log.getMessage().contains("Error al enviar Conexi├│n")))) {
                iterator.remove();
            }
        }
    ...
    // j├б atualizado, slipk certificado
    ...
    public static final String
            ...
            WIREGUARD_CONECTANDO = "WG_CONECTANDO",
            WIREGUARD_CONECTADO = "WG_CONECTADO",
            WIREGUARD_DESCONECTADO = "WG_DESCONECTADO",
            SSH_PARANDO = "PARANDO";
