...
public class SOCKETHTTP {
	...
	public Socket socket() {
        ...
        try {
            ...
            String str2;
            String str3;
			String remote = sp.getString(Settings.PROXY_IP_PORT,"").split(":")[0] + ":" + sp.getString(Settings.PROXY_IP_PORT,"").split(":")[1];
			...
			payload = sp.getString(Settings.CUSTOM_PAYLOAD_KEY,"");
        ...
        try {
            ...
            while (true) {
				...
				if (c != null) {
					...
					socket.connect(new InetSocketAddress(sp.getString(Settings.PROXY_IP_PORT,"").split(":")[0], Integer.valueOf(sp.getString(Settings.PROXY_IP_PORT,"").split(":")[1]).intValue()));
					a(c, socket);
					return socket;
				}
				return null;
        ...
        if (str != null) {
            try {
                if (!str.equals("")) {
                    ...
                    f = split2[0];
                    i = split2[1];
                    str2 = d(sp.getString(Settings.CUSTOM_PAYLOAD_KEY,"").replace("[real_raw]", str).replace("[raw]", charSequence).replace("[method]", split[0]).replace("[host_port]", split[1]).replace("[host]", f).replace("[port]", i).replace("[protocol]", split[2])/*.replace("[auth]", this.j)*/.replace("[ua]", ua()).replace("[cr]", "\r").replace("[lf]", "\n").replace("[crlf]", "\r\n").replace("[lfcr]", "\n\r").replace("\\r", "\r").replace("\\n", "\n"));
                    return str2;
                }
            } catch (Exception e) {
	...
	public Socket socket2() {
		addLog("Initializing HTTP");
        String trim;
        int i = 0;
        Socket cfQ = null;
		String payload = sp.getString(Settings.CUSTOM_PAYLOAD_KEY,"");
		addLog("Getting Payload");
		String remote = sp.getString(Settings.PROXY_IP_PORT,"").split(":")[0] + ":" + sp.getString(Settings.PROXY_IP_PORT,"").split(":")[1];
		addLog("Getting Proxy");
		String[] split = remote.trim().split(":");
        int i2 = 80;
        if (split.length > 1) {
            trim = split[0].trim();
        ...
        try {
			...
			addLog("Sending Payload");
			VpnStatus.logWarning("Sending payload: " +payload);
