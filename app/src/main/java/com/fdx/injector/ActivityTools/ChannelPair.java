...
import com.fdx.injector.ActivityTools.Channel.Status;
...
public class ChannelPair implements ChannelListener {
	...
	private static final String CONNECT_OK = "HTTP/1.0 200 Connection Established" + CRLF + "Proxy-agent: KissProxy"
			+ CRLF + CRLF;
	...
	public void handleKey(SelectionKey key) {
		...
		if (requestChannel != null && key.equals(requestChannel.getSelectionKey())) {
			requestChannel.read();
		} else if (responseChannel != null && key.equals(responseChannel.getSelectionKey())) {
	...
	private boolean connResponse(Channel channel) {
		try {
			...
			if ("CONNECT".equals(method)) {
				...
				stringBuffer.append(url).append(" ").append(channel.getProtocol()).append(CRLF);
				Map<String, String> headers = channel.getHeaders();
				Iterator<String> iterator = headers.keySet().iterator();
				while (iterator.hasNext()) {
					String key = iterator.next();
					stringBuffer.append(key).append(": ").append(headers.get(key)).append(CRLF);
				}
	...
	public void onHeaders(Channel channel) {
		...
		if (channel.isRequest()) {
			...
			for (Entry<String, String> h : channel.getHeaders().entrySet()) {
				stringBuffer.append(h.getKey()).append(": ").append(h.getValue()).append(CRLF);
			}
			stringBuffer.append(CRLF);
			byte[] sendBytes = stringBuffer.toString().getBytes();
