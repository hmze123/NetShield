...
public class OpenVpnManagementThread implements Runnable, OpenVPNManagement {
    ...
    private Runnable orbotStatusTimeOutRunnable =
            new Runnable() {
                @Override
                public void run() {
                    sendProxyCMD(
                            ...
                            "127.0.0.1",
                            Integer.toString(OrbotHelper.SOCKS_PROXY_PORT_DEFAULT),
                            false);
                    OrbotHelper.get(mOpenVPNService).removeStatusCallback(statusCallback);
                }
            };
    private OrbotHelper.StatusCallback statusCallback =
            new OrbotHelper.StatusCallback() {
                ...
                public void onDisabled(Intent intent) {
                    VpnStatus.logWarning(
                            "Orbot integration for external applications is disabled. Waiting %ds"
                                + " before connecting to the default port. Enable external app"
                                + " integration in Orbot or use Socks v5 config instead of Orbot to"
                                + " avoid this delay.");
                }
            };
            ...
            try {
                mServerSocketLocal.bind(
                        new LocalSocketAddress(
                                socketName, LocalSocketAddress.Namespace.FILESYSTEM));
            } catch (IOException e) {
    ...
    private void handleHold(String argument) {
      ...
      //  int waittime = Integer.parseInt(argument.split(":")[1]);
	    ...
	    if (shouldBeRunning()) {
            ...
            mResumeHandler.postDelayed(mResumeHoldRunnable, waittime * 500);
            if (waittime > 2)
    ...
    private void releaseHoldCmd() {
        ...
        if ((System.currentTimeMillis() - mLastHoldRelease) < 100) {
            try {
                Thread.sleep(10);
            } catch (InterruptedException ignored) {
            }
    ...
    private void sendProxyCMD(
            Connection.ProxyType proxyType, String proxyname, String proxyport, boolean usePwAuth) {
        ...
        switch (needed) {
                    ...
                    */
                    ...
                    if (routeparts.length == 5) {
                        ...
                        mOpenVPNService.addRoute(
                                routeparts[0], routeparts[1], routeparts[2], routeparts[4]);
                    } else if (routeparts.length >= 3) {
                        mOpenVPNService.addRoute(routeparts[0], routeparts[1], routeparts[2], null);
                    } else {
                        VpnStatus.logError(
                                "Unrecognized ROUTE cmd:"
                                        + Arrays.toString(routeparts)
                                        + " | "
                                        + argument);
                    }
            ...
            case "IFCONFIG":
                ...
                mOpenVPNService.setLocalIP(
                        ifconfigparts[0], ifconfigparts[1], mtu, ifconfigparts[3]);
                break;
            case "IFCONFIG6":
    ...
    private boolean sendTunFD(String needed, String extra) {
        if (!extra.equals("tun")) {
            ...
            VpnStatus.logError(
                    String.format(
                            "Device type %s requested, but only tun is possible with the Android"
                                + " API, sorry!",
                            extra));
    ...
    private void processPWCommand(String argument) {
        ...
        if (pw != null) {
            ...
            VpnStatus.logError(
                    String.format(
                            "Openvpn requires Authentication type '%s' but no password/key"
