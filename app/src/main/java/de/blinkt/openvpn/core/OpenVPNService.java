...
public class OpenVPNService extends VpnService
	...
	private String TAG = "CHZOpenVpnService";
	...
	PendingIntent getContentIntent() {
		try {
			if (mNotificationActivityClass != null) {
				...
				String typeStart = Objects.requireNonNull(mNotificationActivityClass.getField("TYPE_START").get(null))
						.toString();
	...
	public void onRevoke() {
		if (SOCKETService.isRunning) {
			startService(new Intent(this, SOCKETService.class).setAction(SOCKETService.FLAG_VPN_STOP));
		}
	...
	private String createNotificationChannel(String channelId) {
		if (lastChannel == null || lastChannel.isEmpty()) {
			NotificationChannel chan = new NotificationChannel(channelId, getString(R.string.channel_name_background),
	...
	private void showNotification(final String msg, String tickerText, @NonNull String channel, long when,
		...
		nBuilder.setSmallIcon(R.drawable.ic_leaf);
	...
	PendingIntent getGraphPendingIntent() {
		...
		intent.setComponent(new ComponentName(this, getPackageName() + ".SMainActivity"));
	...
	public int onStartCommand(Intent intent, int flags, int startId) {
		...
		showNotification(VpnStatus.getLastCleanLogMessage(this), VpnStatus.getLastCleanLogMessage(this),
	...
	public ParcelFileDescriptor openTun() {
		...
		// Debug.startMethodTracing(getExternalFilesDir(null).toString() + "/opentun.trace", 40*
		// 1024 * 1024);
		...
		if ("samsung".equals(Build.BRAND) && Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP
			...
			try {
				...
				if (!dnsIncluded) {
					String samsungwarning = String.format("Warning Samsung Android 5.0+ devices ignore DNS servers"
							+ " outside the VPN range. To enable DNS resolution a route"
							+ " to your DNS Server (%s) has been added.", mDnslist.get(0));
		...
		if ((!mRoutes.getNetworks(false).isEmpty() || !mRoutesv6.getNetworks(false).isEmpty())
			...
			VpnStatus.logInfo("VPN lockdown enabled (do not allow apps to bypass VPN) enabled. Route"
					+ " exclusion will not allow apps to bypass VPN (e.g. bypass VPN for local" + " networks)");
		}
		if (mDomain != null)
			builder.addSearchDomain(mDomain);
		...
		try {
			...
			Log.i(TAG, "tun builder");
			ParcelFileDescriptor tun = builder.establish();
			SOCKETService.protect(OpenVPNService.this);
			if (tun == null)
	...
	private void setAllowedVpnPackages(Builder builder) {
		...
		if (profileUsesOrBot)
			VpnStatus.logDebug("VPN Profile uses at least one server entry with Orbot. Setting up VPN so that"
					+ " OrBot is not redirected over VPN.");
	...
	public void addRoute(String dest, String mask, String gateway, String device) {
		...
		if (mLocalIP == null) {
			VpnStatus.logError("Local IP address unset and received. Neither pushed server config nor local"
					+ " config specifies an IP addresses. Opening tun device is most likely" + " going to fail.");
			return;
		}
	...
	public void updateState(String state, String logmessage, int resid, ConnectionStatus level, Intent intent) {
...

		{
			...
			if (level == LEVEL_CONNECTED) {
				if (MainApp.sp.getBoolean(Settings.HOT_SPOT, false)) {
					if (ResetDefault.getSockIP() != null) {
						...
						this.mh.start();
						VpnStatus.logWarning("<font color='#00FF7F'>Hotspot/Tether Enabled: " + ResetDefault.getSockIP()
								+ ":9001</font>");
					} else {
						VpnStatus.logWarning(
								"<font color='#FF5733'>Please turn on MOBILE Hotspot before use this</font>");
					}
				}
			}
			...
			showNotification(VpnStatus.getLastCleanLogMessage(this), VpnStatus.getLastCleanLogMessage(this), channel, 0,
					level, intent);
		}
	}
	...
	public void trigger_sso(String info) {
		...
		if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN) {
			nbuilder.setUsesChronometer(true);
			nbuilder.setPriority(PRIORITY_MAX);
		}
		if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP)
	...
	public class myhandler extends Thread {
		...
		public myhandler(OpenVPNService myService, int i2) {
			this.j = i2;
		}
