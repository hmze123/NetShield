...
public class ConfigParser {
    ...
    private Pair<Connection, Connection[]> parseConnectionOptions(Connection connDefault) throws ConfigParseError {
		...
		/*if (remotes != null){
				MainApp.sp.edit().putString(Settings.OVPN_HOST, remotes.get(2).toString()).apply();
		}*/
        ...
        for (Vector<String> remote : remotes) {
            ...
            switch (remote.size()) {
                ...
                case 3:
					...
					MainApp.sp.edit().putString(Settings.OVPN_PORT, remote.get(2)).apply();
                case 2:
                    connections[i].mServerName = remote.get(1);
					MainApp.sp.edit().putString(Settings.OVPN_HOST, remote.get(1)).apply();
            }
            i++;
        }
